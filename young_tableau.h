...
/// <summary>
/// Young tableau data structure implementation
/// Young tableau is a table, where rows and columns are sorted
/// Empty cells are initialized with infinity valued, which is user defined (can be anything)
/// Proper infinity value must be passed, this allows for creating tableaus that are reversed, improper values will result with improper output
/// This tabelau allows for  containment of non integer elements, such as strings, but proper comparator and infinity value must be passed
/// </summary>
...
public:
	...
	young_tableau(const size_t& width, const size_t& height, const T& infinity_val,
		T* arr, const size_t& arr_size) : _width(width), _height(height), infinity(infinity_val) {
		fill(arr, arr_size);
	}
